...
export class ReactivateSubscriptionReuseForm {
  ...
  selectSubscriber(subscriber: AuthSubscriber) {
    ...
    this.loadReactivatableSubscriptions(subscriber.id);
  }

  loadReactivatableSubscriptions(subscriberId: number) {
    this.subscriptionService.getAllReactivatableSubscription(subscriberId).subscribe({
      next: (response) => {
