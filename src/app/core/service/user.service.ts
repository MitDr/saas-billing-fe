...
export class UserService {
  private apiUrl = 'http://localhost:8080/api/v1/admin/users'; // thay bằng base URL thật

  private readonly DEBUG_BEARER_TOKEN =
    '';
  ...
  getUsers(page: number = 0, size: number = 5): Observable<ListData<User>> {
    const headers = new HttpHeaders({
      Authorization: `Bearer ${this.DEBUG_BEARER_TOKEN}`
    });
    ...
    return this.http.get<any>(this.apiUrl, {headers, params}).pipe(
      map(response => {
        ...
        return response.data as ListData<User>;
      }),
      catchError(error => {
